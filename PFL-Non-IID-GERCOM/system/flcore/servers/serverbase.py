...
# ***********************************************************************************************************************************
    def select_clients(self):
        ...
        if (self.current_round == 0) and (self.entropy == 1):
    ...
    def perc_select_cluster(self):
        ...
        for clients in copy_clients_cluster.values():
            ...
            if self.entropy == 1:
                weigths_clients = [client.entropy if not np.isnan(client.entropy) else 0 for client in clients]
                weigths_clients = np.array(weigths_clients)

            if np.isnan(weigths_clients).any():
                # Substituir NaN por 0
                weigths_clients[np.isnan(weigths_clients)] = 0
...
# ***********************************************************************************************************************************
    ...
    def evaluate(self, acc=None, loss=None, nnc = 0):
        ...
        if nnc == 0:
            ...
            with open(f"{self.algorithm}_metrics_{self.entropy}_{self.weigths_clients}_{self.fall_tolerance}_nnc.txt", "a") as arquivo:
                arquivo.write(f"{test_acc}, {train_loss}, ")
